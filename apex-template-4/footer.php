...
<footer class="pt-5 overflow-hidden position-relative" id="contact">
    <!-- SVG decoration -->
	<figure class="position-absolute" style="margin-top: -10.6rem; margin-left: -11rem; z-index: -1;">
    ...
    <div class="container pt-5">
        <div class="row">
            <div class="col-lg-8 mx-auto align-items-center text-start mb-4 pt-5">
                <h3 class="fw-bold mb-2">Organised by UCCI Ltd.</h3>
                <p class="text-muted mt-4 mb-2">The Utkal Chamber of Commerce and Industry Limited (UCCIL) is the Apex Industry Chamber in the State of Odisha, constituted under the iconic and visionary leadership of Late Sri Biju Patnaik in the year 1964. UCCIL comprises of three sectors of the economy i.e., Industry, Trade & Services, including all Large, Medium, Small and Micro Enterprise segment, with around 1000 active members, having completed more than 60 years of its eventful existence.</p>
                <p class="text-muted">
                    <b>Supported By:</b> <img src="https://utkalchamber.in/ResurgentOdisha/assets/images/seal-odisha.png" alt="Logo Odisha Govenment" style="padding-right: 5px;"> <img src="https://utkalchamber.in/ResurgentOdisha/assets/images/odisha-opportunity.png" alt="Logo Odisha New Opportunities" style="padding-right: 5px;"> <img src="https://utkalchamber.in/ResurgentOdisha/assets/images/msme-log-footer.png" alt="Logo MSME">
                </p>
            </div>   
            <div class="col-lg-4 mb-4 contact-div">
                <h6 class="mb-2 fw-bold" style="padding-bottom:.03rem;">Contact Details</h6>
                <!-- <div class="d-flex small text-muted contact-details">
                    <i class="bi bi-pin-map-fill text-primary contact-icons"></i> The Utkal Chamber of Commerce &amp; Industry Ltd.<br>N/6, IRC Village, Nayapalli<br>Bhubaneswar - 751015, Odisha
                </div>
                <div class="d-flex small text-muted contact-details">
                    <i class="bi bi-phone text-primary contact-icons"></i>+91 9437003344 /  9861123728 / 9861010407
                </div>
                <div class="d-flex small text-muted contact-details">
                    <i class="bi bi-envelope text-primary contact-icons"></i><a href="mailto:resurgentodisha@utkalchamber.in" class="text-primary-hover">resurgentodisha@utkalchamber.in</a>
                </div>
                <div class="d-flex small text-muted contact-details">
                    <i class="bi bi-globe2 text-primary contact-icons"></i><a href="http://www.utkalchamber.in" class="text-primary-hover">www.utkalchamber.in</a>
                </div> -->
                <ul class="list-group list-group-borderless mb-2 mb-sm-4">
                    <li class="list-group-item d-flex small mb-0">
                        <i class="bi bi-pin-map-fill text-danger contact-icons"></i> The Utkal Chamber of Commerce &amp; Industry Ltd.<br>N/6, IRC Village, Nayapalli<br>Bhubaneswar - 751015, Odisha
                    </li>
                    <li class="list-group-item d-flex small mb-0">
                        <i class="bi bi-phone text-danger contact-icons"></i>+91 9437003344 /  9861123728 / 9861010407
                    </li>
                    <li class="list-group-item d-flex small mb-0 text-muted">
                        <i class="bi bi-envelope text-danger contact-icons"></i><a href="mailto:resurgentodisha@utkalchamber.in" class="text-primary-hover">resurgentodisha@utkalchamber.in</a>
                    </li>
                    <li class="list-group-item d-flex small mb-0 text-muted">
                        <i class="bi bi-globe2 text-danger contact-icons"></i><a href="http://www.utkalchamber.in" class="text-primary-hover">www.utkalchamber.in</a>
                    </li>
                </ul>
            </div>
            ...
            <div class="col-12 text-muted">
                <hr class="my-0">
                <div class="py-4">
                    Copyrights © 2024 <a href="https://www.utkalchamber.in/" class="text-primary-hover">Utkal Chamber of Commerce &amp; Industry Limited</a>. 
                </div>
            </div>
        </div>   
    </div>  
...
<script>
    ...
    $(window).scroll(function(){
        if ($(window).width() > 1024) {
            if ($(this).scrollTop() > 400) {
                $('#apex-newClass').addClass('sticky-top');
                $('#apex-newClass').removeClass('header-absolute');
                $('#apex-newClass').addClass('header-sticky');
                $('#apex-newClass').addClass('logo-transition');
                $('#newClass').addClass('py-3');
                $('#navbarSupportedContent').addClass('pt-2');
                $('#newClass').removeClass('nav-container');
            } else {
                $('#apex-newClass').removeClass('sticky-top');
                $('#apex-newClass').addClass('header-absolute');
                $('#apex-newClass').removeClass('header-sticky');
                $('#apex-newClass').removeClass('logo-transition');
                $('#newClass').removeClass('py-3');
                $('#navbarSupportedContent').removeClass('pt-2');
                $('#newClass').addClass('nav-container');
            }
        }
    ...
    $(document).on('click', '.custom-dropdown-btn', function() {
        ...
        if (tag == 'SPAN') {
            if ($(this).find(".custom-dropdown-icon i").hasClass("bi-dash")) {
                $(this).find(".custom-dropdown-icon i").removeClass("bi-dash").addClass("bi-plus");
            } else {
                $(this).find(".custom-dropdown-icon i").removeClass("bi-plus").addClass("bi-dash");
            }
        } else {
            if ($(this).find(".custom-dropdown-icon").hasClass("bi-dash")) {
                $(this).find(".custom-dropdown-icon").removeClass("bi-dash").addClass("bi-plus");
            } else {
                $(this).find(".custom-dropdown-icon").removeClass("bi-plus").addClass("bi-dash");
            }
        }
    });
    ...
    const icon = menuToggle.querySelector('i'); // The icon inside the button

    menuToggle.addEventListener('click', () => {
    submenu.classList.toggle('open');  // Toggle the 'open' class on the submenu
    
    // Toggle the icon based on the submenu state
    if (submenu.classList.contains('open')) {
        icon.classList.remove('bi-list');
        icon.classList.add('bi-x');  // Show the 'x' icon when the menu is open
    } else {
        icon.classList.remove('bi-x');
        icon.classList.add('bi-list');  // Show the 'list' icon when the menu is closed
    }
    });
